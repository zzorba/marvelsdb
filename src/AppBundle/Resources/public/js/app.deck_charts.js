(function app_deck_charts(deck_charts, $) {
	...
	deck_charts.chart_type = function chart_type() {
		...
		var draw_deck = app.deck.get_physical_draw_deck();
		draw_deck.forEach(function (card) {
			if(!types[card.type_code]) types[card.type_code] = { code: card.type_code, name: card.type_name, count: 0};
			types[card.type_code].count += card.indeck;
		})

		var data = [];
		_.each(_.values(types), function (_type) {
			data.push({
				name: _type.name,
				label: '<span class="icon icon-'+_type.code+'"></span>',
				color: type_colors[_type.code],
				y: _type.count
			});
		})
